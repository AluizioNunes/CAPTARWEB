...
export default function Ativistas() {
  ...
  const load = async () => {
    try {
      setLoading(true)
      const schema = await api.getAtivistasSchema()
      const defaultOrder = [
        'id',
        'nome',
        'tipo_apoio',
      ...
      const res = await api.getAtivistas()
      const rows = res.rows || []
      setData(rows)
      ...
      const visKey = `ativistas.columns.visible.${(user as any)?.usuario || 'default'}`
      const savedVis = localStorage.getItem(visKey)
      setVisibleCols(savedVis ? JSON.parse(savedVis) : visDefault)
      
      const orderKey = `ativistas.columns.order.${(user as any)?.usuario || 'default'}`
      const savedOrder = localStorage.getItem(orderKey)
      setOrderedKeys(savedOrder ? JSON.parse(savedOrder) : defaultOrder.filter(n => byName[n]))
    } catch (e: any) {
      message.error(e?.response?.data?.detail || 'Erro ao carregar ativistas')
    } finally {
      setLoading(false)
    }
  }
      ...
      <Space>
        ...
        <Button type="text" danger icon={<IconDelete />} title="DELETAR" onClick={async () => {
          try {
            ...
            await api.deleteAtivista(record.id)
            message.success('ATIVISTA DELETADO')
            await load()
          } catch (e: any) {
            message.error(e?.response?.data?.detail || 'ERRO AO DELETAR ATIVISTA')
          }
        }} />
      </Space>
    )
    ...
    <motion.div className="page-container" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 12 }}>
        <Space>
          ...
          <Button shape="circle" icon={<IconAdd />} style={{ background: '#FFD700', borderColor: '#FFD700', color: '#000' }} onClick={() => { setEditing(null); setModalOpen(true) }} title="NOVO ATIVISTA" />
        </Space>
      </div>
      <style>{`
